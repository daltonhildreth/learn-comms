...
public:
    ...
    template <typename T> uint16_t create(T&&);

    // T must be a component type; also comp_id is NOT Entity::id
    template <typename T> T* get(uint16_t comp_id);

    // T must be a component type
    template <typename T> T* get(const Entity&);

    // T must be a component type
    template <typename T> void attach(Entity&, uint16_t);

    template <typename T> void for_(std::function<void(T&, Entity&)>);

private:
    std::unordered_map<uint16_t, Entity> _entity_table;
    uint16_t _entity_count;

    template <typename T> DataTable<T>& _table();

    template <typename T> unsigned _comp();
    ...
    DataTable<CommComp> _comm_table;
};
