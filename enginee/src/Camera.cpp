...
float Camera::fov() { return _fov; }

void Camera::aspect(float aspect) {
    _aspect = aspect;
    _dirty = true;
}
float Camera::aspect() { return _aspect; }

void Camera::near(float near) {
    _near = near;
    _dirty = true;
}
float Camera::near() { return _near; }

void Camera::far(float far) {
    _far = far;
    _dirty = true;
}
float Camera::far() { return _far; }
...
glm::vec3 Camera::look_dir() { return _look_dir; }
glm::vec3 Camera::up() { return _up; }
glm::vec3 Camera::right() { return glm::cross(_look_dir, _up); }
